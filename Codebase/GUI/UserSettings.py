...
res=ExecuteCommand("SELECT stylesheet, mintaskdispheight, minsecdispheight, projectminheight , sidebarfactor, setduedatetoday FROM settings WHERE def=1")
if len(res)==1:
    Settings=res[0]
else:
    Settings=Defaults
...
Stylesheet,MinTaskDispHt,MinSecDispHt,ProjMinHt,sidebarfactor,duedatebehaviour=Settings
